...
class CarlaEnv(gym.Env):
    def __init__(
        ...
        self.IM_SHAPE = (256, 256)  # width, height
    ...
    def reset(self):
        ...
        self.cam = self.world.spawn_actor(self.cam_bp, carla.Transform(carla.Location(x=0, z=8), carla.Rotation(-30, 0, 0)), attach_to=self.vehicle)
    ...
    def step(self, action):
        ...
        action = self.mpc_control(states)
        # obs = self.get_obs()
        # obs = None
        if self.cam_img is not None and self.playing:
            ...
            if key == 27:
                assert False
        self.frame_step += 1
        throttle = self.pid(states[-2] + self.des_v)
        if throttle < 0:
            action = carla.VehicleControl(throttle=0.0, brake=-throttle, steer=float(action))
        else:
            action = carla.VehicleControl(throttle=throttle, steer=float(action), brake=0.0)
    ...
    def make_mpc(self):
        ...
        lterm = (
            0.1 * model_mpc.x["x0"] ** 2
            + theta1 * model_mpc.u["u"] ** 2
